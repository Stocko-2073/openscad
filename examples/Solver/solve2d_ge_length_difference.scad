// solve2d_ge_length_difference.scad — bound |ab| from below relative to |cd|.
// |cd| is fixed at 5. |ab| - |cd| >= 1 forces |ab| >= 6.
// Without the inequality, |ab| would be 4 (from a strong distance pull
// via the seed); the loop should activate the inequality and push |ab| to
// exactly 6.

sol = solve2d([
  point("a", at = [0, 0]), con_fixed("a"),
  point("b", at = [4, 0]), con_horizontal("a", "b"),
  point("c", at = [0, 10]), con_fixed("c"),
  point("d", at = [5, 10]), con_horizontal("c", "d"),
  con_distance("c", "d", 5),
  con_ge_length_difference("a", "b", "c", "d", 1),  // |ab| - |cd| >= 1
]);
...
bx = pt(sol, "b")[0];
assert(abs(abs(bx) - 6) < 1e-4, str("expected |b.x| == 6, got ", bx));
