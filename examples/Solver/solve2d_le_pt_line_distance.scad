// solve2d_le_pt_line_distance.scad — point bounded by signed distance to a line.
//
...
// con_le_pt_line_distance("p","a","b", 3) requires signed_dist(p) <= 3,
// i.e. p.y >= -3.
//
// p is seeded at [5, -8] (below the line, signed_dist = 8 > 3: violation).
// The inequality activates and pulls p up to p.y = -3 (signed_dist = 3).

sol = solve2d([
  point("a", at = [0, 0]), con_fixed("a"),
  point("b", at = [10, 0]), con_fixed("b"),
  point("p", at = [5, -8]),                           // seed below x-axis
