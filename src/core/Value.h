 ...
 * -- In some cases a copy of a Value is necessary or unavoidable, in which case Value::clone() can be
 * used.
...
public:
   ...
   * by treating their elements as elements of their parent, traversable via VectorType's custom
   * iterator.
   * -- An embedded vector should never exist "in the wild", only as a pseudo-element of a parent vector.
   *    Eg "Lc*" Expressions return Embedded Vectors but they are necessarily child expressions of a
   * Vector expression.
   ...
   * -- Any loops through VectorTypes should prefer automatic range-based for loops eg: for(const auto&
   * value : vec) { ... } which make use of begin() and end() iterators of VectorType.
   * https://en.cppreference.com/w/cpp/language/range-for
   * -- Moving a temporary Value of type VectorType or EmbeddedVectorType is always safe,
   *    since it just moves the shared_ptr in its possession (which might be a copy but that doesn't
   * matter). Additionally any VectorType can be converted to an EmbeddedVectorType by moving it into
  ...
  protected:
    ...
    struct VectorObject {
      ...
      vec_t vec;
      size_type embed_excess =
        0;  // Keep count of the number of embedded elements *excess of* vec.size()
      class EvaluationSession *evaluation_session =
        nullptr;  // Used for heap size bookkeeping. May be null for vectors of known small maximum size.
  ...
  public:
    ...
    // such that calling code will only receive references to "true" elements (i.e. NOT
    // EmbeddedVectorTypes). Also tracks the overall element index. In case flattening occurs during
    // iteration, it can continue based on that index. (Issue #3541)
    class iterator
    {
    private:
...
private:
  Value()
    : value(UndefType()) {}  // Don't default construct empty Values.  If "undefined" needed, use
